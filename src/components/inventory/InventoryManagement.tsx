...
export default function InventoryManagement({ panelType }: InventoryManagementProps) {
  ...
  const [adjustModalProduct, setAdjustModalProduct] =
    useState<InventoryProduct | null>(null);
  const [detailsProduct, setDetailsProduct] = useState<InventoryProduct | null>(null);
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/v1/inventory/products?${params}`, {
        headers: authHeaders,
        cache: "no-store",
      });
  ...
  const openPurchaseModal = (product: InventoryProduct) => {
    ...
    setPurchaseForm({
      ...
      unitCostPrice: String(product.lastPurchaseCost || product.costPrice || ""),
  ...
  const loadDetails = async (product: InventoryProduct) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/v1/inventory/products/${product.id}`, {
        headers: authHeaders,
        cache: "no-store",
      });
  ...
  const handleSeedOpeningStock = async () => {
    if (!confirm("Create opening stock batches for existing products that have stock but no batch?")) {
      return;
    }

    try {
      setActionLoading("seed");
      const res = await fetch(`${API_BASE}/api/v1/inventory/seed-opening-stock`, {
        method: "POST",
        headers: authHeaders,
      });
      const data = await res.json();
  ...
  const endEntry = pagination.total === 0 ? 0 : Math.min(pagination.page * pagination.limit, pagination.total);
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-400">
              Maintain purchase batches, stock, cost, selling price, and FIFO inventory.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            >
              <FaSyncAlt className={actionLoading === "seed" ? "animate-spin" : ""} />
              Seed Opening Stock
            </button>

            <a
              href={panelType === "admin" ? "/admin/reports/profit-loss" : "/moderator/reports/profit-loss"}
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="relative">
              <FaSearch className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-neutral-500" size={13} />
              <input
                value={search}
                onChange={(event) => {
      ...
      <div className="overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-950">
            ...
            <tbody className="divide-y divide-neutral-900">
              {loading ? (
                ...
                products.map((product) => {
                  const avgCost = safeNumber(product.averageCost || product.costPrice);
                  const profitPerUnit = safeNumber(product.sellingPrice) - avgCost;

                  return (
                    <tr key={product.id} className="align-top transition hover:bg-white/[0.02]">
                      <td className="px-4 py-4">
                        <div className="flex min-w-[260px] items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <span className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-[11px] font-bold ${getStockBadgeClass(product.stockStatus)}`}>
                              {product.stockStatus?.replaceAll("_", " ")}
                            </span>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4 font-black text-white">{product.stock || 0}</td>
                      <td className="px-4 py-4 text-neutral-300">{formatPrice(product.mrp || 0)}</td>
                      <td className="px-4 py-4 text-neutral-300">{formatPrice(avgCost)}</td>
                      <td className="px-4 py-4 text-neutral-300">{formatPrice(product.sellingPrice || 0)}</td>
                      <td className="px-4 py-4 font-bold text-neutral-200">{formatPrice(product.stockValue || 0)}</td>
                      <td className={`px-4 py-4 font-black ${profitPerUnit >= 0 ? "text-emerald-300" : "text-red-300"}`}>
        ...
        <div className="flex flex-col gap-3 border-t border-neutral-800 px-4 py-4 text-sm text-neutral-400 sm:flex-row sm:items-center sm:justify-between">
          <p>
            Showing <span className="font-bold text-white">{startEntry}</span> to{" "}
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Quantity">
            <input className={inputClass()} value={purchaseForm.quantity} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, quantity: event.target.value }))} placeholder="Example: 10" type="number" min="1" />
          </Field>
          <Field label="Unit Cost Price">
            <input className={inputClass()} value={purchaseForm.unitCostPrice} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, unitCostPrice: event.target.value }))} placeholder="Example: 850" type="number" min="0" />
          </Field>
          <Field label="MRP">
            <input className={inputClass()} value={purchaseForm.mrp} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, mrp: event.target.value }))} placeholder="Optional" type="number" min="0" />
          </Field>
          <Field label="Selling Price">
            <input className={inputClass()} value={purchaseForm.sellingPrice} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, sellingPrice: event.target.value }))} placeholder="Optional" type="number" min="0" />
          </Field>
          <Field label="Supplier Name">
            <input className={inputClass()} value={purchaseForm.supplierName} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, supplierName: event.target.value }))} placeholder="Optional" />
          </Field>
          <Field label="Supplier Phone">
            <input className={inputClass()} value={purchaseForm.supplierPhone} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, supplierPhone: event.target.value }))} placeholder="Optional" />
          </Field>
          <Field label="Supplier Invoice">
            <input className={inputClass()} value={purchaseForm.supplierInvoiceNumber} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, supplierInvoiceNumber: event.target.value }))} placeholder="Optional" />
          </Field>
          <Field label="Purchase Date">
            <input className={inputClass()} value={purchaseForm.purchaseDate} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, purchaseDate: event.target.value }))} type="date" />
          </Field>
          <div className="sm:col-span-2">
            <Field label="Note">
              <textarea className="min-h-24 w-full rounded-2xl border border-neutral-800 bg-black px-4 py-3 text-sm text-white outline-none transition placeholder:text-neutral-600 focus:border-orange-500" value={purchaseForm.note} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, note: event.target.value }))} placeholder="Optional note" />
            </Field>
          </div>
          <label className="flex items-center gap-3 rounded-2xl border border-neutral-800 bg-black p-4 text-sm font-bold text-white sm:col-span-2">
            <input type="checkbox" checked={purchaseForm.updateProductPrice} onChange={(event) => setPurchaseForm((prev) => ({ ...prev, updateProductPrice: event.target.checked }))} className="h-4 w-4 accent-orange-500" />
            Update product MRP / selling price / latest cost with this purchase
          </label>
        </div>

        <div className="mt-5 flex justify-end gap-2">
          <button type="button" onClick={() => setPurchaseModalProduct(null)} className="rounded-2xl border border-neutral-800 px-4 py-2.5 text-sm font-bold text-neutral-300 hover:text-white">
            Cancel
          </button>
          <button type="button" onClick={submitPurchase} disabled={actionLoading === "purchase"} className="rounded-2xl bg-orange-600 px-5 py-2.5 text-sm font-black text-white hover:bg-orange-700 disabled:opacity-60">
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Adjustment Type">
            <select className={inputClass()} value={adjustForm.type} onChange={(event) => setAdjustForm((prev) => ({ ...prev, type: event.target.value as InventoryMovementType }))}>
              <option value="ADJUSTMENT_OUT" className="bg-black">Stock Decrease</option>
              <option value="DAMAGE" className="bg-black">Damage</option>
              <option value="LOSS" className="bg-black">Loss</option>
              <option value="ADJUSTMENT_IN" className="bg-black">Stock Increase</option>
            </select>
          </Field>
          <Field label="Quantity">
            <input className={inputClass()} value={adjustForm.quantity} onChange={(event) => setAdjustForm((prev) => ({ ...prev, quantity: event.target.value }))} placeholder="Example: 2" type="number" min="1" />
          </Field>
          {adjustForm.type === "ADJUSTMENT_IN" && (
            <Field label="Unit Cost Price">
              <input className={inputClass()} value={adjustForm.unitCostPrice} onChange={(event) => setAdjustForm((prev) => ({ ...prev, unitCostPrice: event.target.value }))} placeholder="Required for stock increase" type="number" min="0" />
            </Field>
          )}
          <div className="sm:col-span-2">
            <Field label="Reason">
              <textarea className="min-h-24 w-full rounded-2xl border border-neutral-800 bg-black px-4 py-3 text-sm text-white outline-none transition placeholder:text-neutral-600 focus:border-orange-500" value={adjustForm.reason} onChange={(event) => setAdjustForm((prev) => ({ ...prev, reason: event.target.value }))} placeholder="Example: Damaged product, manual correction" />
            </Field>
          </div>
        </div>

        <div className="mt-5 flex justify-end gap-2">
          <button type="button" onClick={() => setAdjustModalProduct(null)} className="rounded-2xl border border-neutral-800 px-4 py-2.5 text-sm font-bold text-neutral-300 hover:text-white">
            Cancel
          </button>
          <button type="button" onClick={submitAdjustment} disabled={actionLoading === "adjust"} className="rounded-2xl bg-orange-600 px-5 py-2.5 text-sm font-black text-white hover:bg-orange-700 disabled:opacity-60">
      ...
      <Modal
        ...
        title={detailsProduct ? `Inventory Details: ${detailsProduct.name}` : "Inventory Details"}
        onClose={() => setDetailsProduct(null)}
      >
        {detailsLoading ? (
          <div className="py-10 text-center text-neutral-400">Loading details...</div>
        ) : (
          <div className="space-y-6">
            {detailsProduct && (
              <div className="grid gap-3 sm:grid-cols-4">
                <div className="rounded-2xl border border-neutral-800 bg-black p-4">
                  ...
                  <p className="mt-1 text-xl font-black text-white">{detailsProduct.stock}</p>
                </div>
                <div className="rounded-2xl border border-neutral-800 bg-black p-4">
                  <p className="text-xs text-neutral-500">Average Cost</p>
                  <p className="mt-1 text-xl font-black text-white">{formatPrice(detailsProduct.averageCost || 0)}</p>
                </div>
                <div className="rounded-2xl border border-neutral-800 bg-black p-4">
                  <p className="text-xs text-neutral-500">Selling Price</p>
                  <p className="mt-1 text-xl font-black text-white">{formatPrice(detailsProduct.sellingPrice || 0)}</p>
                </div>
                <div className="rounded-2xl border border-neutral-800 bg-black p-4">
                  <p className="text-xs text-neutral-500">Stock Value</p>
                  <p className="mt-1 text-xl font-black text-white">{formatPrice(detailsProduct.stockValue || 0)}</p>
                </div>
              </div>
            )}
                ...
                <table className="min-w-full divide-y divide-neutral-800 text-left text-xs">
                  ...
                  <tbody className="divide-y divide-neutral-900">
                    {detailsBatches.length === 0 ? (
                      <tr><td colSpan={5} className="px-3 py-5 text-center text-neutral-500">No batch found</td></tr>
                    ) : detailsBatches.map((batch) => (
                      <tr key={batch.id}>
                        <td className="px-3 py-3 font-bold text-white">{batch.batchNo}</td>
                        <td className="px-3 py-3 text-neutral-300">{batch.purchaseQuantity}</td>
                        <td className="px-3 py-3 text-neutral-300">{batch.remainingQuantity}</td>
                        <td className="px-3 py-3 text-neutral-300">{formatPrice(batch.unitCostPrice)}</td>
                        <td className="px-3 py-3 text-neutral-400">{formatDate(batch.purchaseDate)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <section>
              <h3 className="mb-3 text-base font-black text-white">Recent Movements</h3>
              <div className="overflow-hidden rounded-2xl border border-neutral-800">
                <table className="min-w-full divide-y divide-neutral-800 text-left text-xs">
                  <thead className="bg-black text-neutral-500">
                    <tr>
                  ...
                  <tbody className="divide-y divide-neutral-900">
                    {detailsMovements.length === 0 ? (
                      <tr><td colSpan={5} className="px-3 py-5 text-center text-neutral-500">No movement found</td></tr>
                    ) : detailsMovements.map((movement) => (
                      <tr key={movement.id}>
                        <td className="px-3 py-3 font-bold text-white">{movementLabel(movement.type)}</td>
                        <td className={`px-3 py-3 font-black ${movement.quantity >= 0 ? "text-emerald-300" : "text-red-300"}`}>{movement.quantity}</td>
                        <td className="px-3 py-3 text-neutral-300">{formatPrice(movement.totalCost || 0)}</td>
                        <td className="px-3 py-3 text-neutral-400">{movement.referenceNo || "-"}</td>
                        <td className="px-3 py-3 text-neutral-400">{formatDate(movement.createdAt)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
